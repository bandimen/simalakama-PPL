<x-layout>
                            ...
                            @if ($mhs)
                                ...
                                @foreach ($mhs as $m)
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                        ...
                                        <td class="px-6 py-4">
                                            <a href="#" class="font-medium hover:underline" data-modal-target="modal-{{ $m->id }}"
                                                data-modal-toggle="modal-{{ $m->id }}">{{ $m->nim }} </a>
                                        </td>
                                        <td class="px-6 py-4">
                                            <a href="#" class="font-medium hover:underline" data-modal-target="modal-{{ $m->id }}"
                                                data-modal-toggle="modal-{{ $m->id }}">
                                                {{ $m->nama }}</a>
                                            <!-- Main modal jika Mhs sudah mengisi IRS-->
                                            @if ($m->irs->isNotEmpty())
                                            <div id="modal-{{ $m->id }}" tabindex="-1" aria-hidden="true"
    ...
    $counterDetail = 0;
@endphp
@foreach ($m->irs as $irs) // Loop through each IRS record
    @if ($irs->irsDetails->isNotEmpty()) // Check if IRS details exist
        @foreach ($irs->irsDetails as $detail)
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                <td class="px-6 py-4">{{ ++$counterDetail }}</td>
                <td class="px-6 py-4">{{ $detail->kodemk }}</td>
                <td class="px-6 py-4">{{ $detail->mataKuliah->nama }}</td>
                <td class="px-6 py-4">{{ $detail->jadwalKuliah->kelas }}</td>
                <td class="px-6 py-4">{{ $detail->mataKuliah->sks }}</td>
                <td class="px-6 py-4">{{ $detail->jadwalKuliah->ruang->nama }}</td>
                <td class="px-6 py-4">{{ $detail->status }}</td>
                <td class="px-6 py-4">
                    @if (isset($detail->dosenPengampuList) && $detail->dosenPengampuList->isNotEmpty())
                        <ul>
    ...
    @else
        <tr>
            <td colspan="8" class="px-6 py-4">Tidak ada mata kuliah.</td>
        </tr>
    @endif
@endforeach
                                                                    </tbody>
                                                        ...
                                                        <div
                                                            ...
                                                            @if ($m->status == 'Belum disetujui')
                                                                <a href="{{ route('setujuiIrs', $m->id) }}">
                                                                    <button data-modal-hide="modal-{{ $m->id }}"
                                                                        type="button"
                                            ...
                                            <div id="modal-{{ $m->id }}" tabindex="-1" aria-hidden="true"
                                                ...
                                                <div class="relative p-4 w-full max-w-6xl max-h-full">
                                                    ...
                                                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                                        ...
                                                        <div class="p-4 md:p-5 space-y-4">
                                                            <div class="relative overflow-x-auto">
                                                                <p>Mahasiswa belum mengisi IRS.</p>
                                                            </div>
